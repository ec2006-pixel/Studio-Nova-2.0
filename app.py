...
from state_manager import get_session, save_session, delete_session
...
ACCESS_TOKEN = "".strip()
PHONE_NUMBER_ID = "1055451567650000"
...
def alertar_esteban(datos):
    ...
    send_whatsapp_message(ESTEBAN_CEL, cuerpo)
...
def procesar_mensaje_bot(numero_usuario, usuario_dice):
    ...
    # --- FLUJO 2. DESCRIPCIÓN DEL NEGOCIO ---
        ...
        if msg_low == '1':
            ...
            respuesta = (
                ...
                "Imagina que tu negocio responde clientes a las 3am, enviía precios automáticos y agenda citas — sin que tú levantes un dedo. ☏\ufe0f\n\n"
    ...
    # --- FLUJO 3. NOMBRE EMPRESA ---
        ...
        respuesta = (
            ...
            "_Ej: Venta de ropa, Taller mecánico, Servicios jurídicos, Panadera, etc._"
        )
    ...
    # --- FLUJO 4. SECTOR/PRODUCTO + GROQ ---
        ...
        if client_groq:
            try:
                ...
                system_prompt = (
                    f"Actua como consultor de ventas de Studio Nova. "
    ...
    # --- FLUJO 5. CIERRE Y RESUMEN ---
        ...
        estado_usuario["presupuesto"] = usuario_dice
            ...
            if body.get('entry') and 'changes' in body['entry'][0]:
                ...
                if 'value' in cambios and 'messages' in cambios['value']:
                    ...
                    if mensaje.get('type') == 'text':
                        ...
                        logger.info(f"Interacción entrante -> NUM: {numero_remitente} | MSG: {texto_mensaje}")
...
if __name__ == '__main__':
    ...
    logger.info(f"🚀 Iniciando Studio Nova Engine en puerto {puerto}")
